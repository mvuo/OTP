    ...
    if (connectionSocket < 0){
      ...
      if (fork() == 0)
      {
        ...
        close(listenSocket);
				...
				// Sends own signature
				if (!sendBuf(connectionSocket, &decode, sizeof decode)) {
					...
					if (!recvBuf(connectionSocket, &sign, sizeof sign)) {
						...
						if (sign != DECODE_SIGN) {
							...
							if (!recvStr(connectionSocket, &cipher)){
								error("Error: Can't receive the ciphertext");
							}
							else {
								...
								if (!recvStr(connectionSocket, &key)) {
									...
									// Applies the key to the ciphertext
									char *plain = oneTimePadCrypt(cipher, key, DECODE);
									if (plain) {
										// Sends the plain text to the client
										if (!sendStr(connectionSocket, plain)) {
											error("Error: Can't send the plain text to the client");
										}
										free(plain);
									}
									free(key);
